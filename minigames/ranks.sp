...
void DisplayRankDetails(int client, const char[] username, DataPack pack)
{
    ...
    FormatEx(buffer, 128, "杀亡比: %.2f  |  爆头率: %.2f%%  |  命中率: %.2f%%", float(data[iKills])/float(data[iDeaths]+1), float(data[iHeadshots] * 100)/float(data[iKills] - data[iKnifeKills] - data[iTaserKills]));panel.DrawText(buffer);
